...
The dedicated `memory` tool uses `memory list|add|replace|remove`. Its read-only `list` action shows the current user/project owner's exact `scope/key` facts and pending candidates for duplicate avoidance, replacement, or removal.
