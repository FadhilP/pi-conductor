...
export default function scoutExtension(pi: ExtensionAPI, runRepoScout = runPi) {
  ...
  pi.registerTool({
    ...
    label: "Repo Scout",
    description:
      "Read-only isolated repository reconnaissance with exact line-range citations and narrow excerpts. Give an observable search or trace, concrete scope anchors, required evidence, and a finite stopping boundary; keep evaluation and final conclusions in the main model. Every call uses a fresh child session; calls are unlimited per original user request.",
    promptSnippet:
      "Map concrete repository paths, symbols, patterns, boundaries, data flow, cross-file impact, exact line ranges, and uncertainty",
    promptGuidelines: [
      "Use repo_scout before editing for non-local repository understanding: implementation, architecture/data flow, or cross-file impact. If no path, package, symbol, pattern, boundary, input, or sink anchor is known, first do one bounded parent-side fd/rg/narrow-read orientation; stop once enough anchors exist. Skip repo_scout for known-file self-contained edits. When needed, call it before mutation tools.",
      "Write each repo_scout task with four parts: observable action (locate/enumerate/compare/trace); concrete anchors or bounded scope; requested evidence (definitions, callers, checks, limits, factual differences, and in-scope gaps); finite stopping boundary allowing directly relevant imports, registries, config, tests, or indirect dispatch. Include only goal constraints affecting scope or interpretation. Scout reports facts including duplication, missing checks, divergent limits, and uncertainty; main model judges quality, cost, severity, exploitability, priority, architecture, and recommendations. Replace 'inspect all packages for token-cost or quality opportunities' with 'Across packages/a and packages/b, trace context assembly, redaction, and truncation; cite definitions, callers, shared helpers, limits, factual differences, and in-scope gaps; stop when relationships needed to evidence those flows are covered.' Replace 'find critical vulnerabilities' with 'Within named auth entrypoints, trace user input to SQL, shell, filesystem, network, deserialization, or secrets; cite checks and in-scope gaps; stop when relevant handlers, sinks, and tests are covered.'",
      "Treat repo_scout citations as the working set. Reread only for an exact edit, stated gap/conflict, or changed repository state; read cited ranges first, expanding only to resolve that need. Do not re-scout merely because implementation was approved; require changed scope/state, unresolved gaps, or missing edit anchors. Before follow-up repo_scout, use one bounded parent-side fd/rg/narrow-read gap pass when useful, then combine related gaps. Fresh child sessions inherit no Scout report: copy relevant prior paths, symbols, facts, constraints, and unresolved gap into task or retryReason; never say only 'using the prior map' or copy the whole report.",
    ],
