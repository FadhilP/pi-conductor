...
Calls are unlimited within one original user prompt and every call uses a fresh child session. An initial self-contained call sends no parent conversation context. A call with `retryReason` is a follow-up and receives a redacted, bounded parent handoff; include relevant prior findings, changed constraints, or parent-side results because child-session history is never reused. Follow up only for a real gap, missing edit anchor, or repository-state change, and combine related gaps into one task.
...
The isolated child loads Scout's private child entrypoint and has read-only `read`, `search_excerpt`, `rg`, `fd`, `grep`, `find`, and `ls`. General host `rg` and `fd` discovery tools are provided separately by pi-discover; Scout does not depend on that package. `search_excerpt` returns deterministic line-numbered matching excerpts plus bounded context in one call, tries `rg` then `grep`, and accepts workspace-relative `path`, `pattern`, optional `glob`, and up to three context lines. Search and path-tool results are capped at 24 KiB and explicitly report omissions; oversized cited search results are sampled deterministically across files instead of keeping only the head. Focused reads and path listings remain head-bounded. `rg` keeps its match-count protection. The child has no shell, mutation tools, other extensions, skills, or context files. There is no hard child turn cap. Activity history retains at most 100 events. Timeouts fail nonfatally; retry with a focused follow-up task.
